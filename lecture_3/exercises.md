...
3. Ако имате 1000 такива изречения, кой подход ще има по-дълги редици от токени?

**Въпрос:** Ако контекстния прозорец е 512 токена, колко "Hello world" изречения се събират при всеки подход?

---

## Упражнение 3: BPE сливане (merge операция)
...
1. По думи: 2 токена ("Hello", "world")
2. По символи: 11 токена ('H','e','l','l','o',' ','w','o','r','l','d')
3. По символи ще има по-дълги редици

При 512 токена:

- По думи: 512/2 = 256 изречения
- По символи: 512/11 ≈ 46 изречения

## Решение 3

1. Има съвпадение между (o, w) и (l, o) с честота 7. Обикновено се избира първата по азбучен ред или първата намерена: (l, o).
...
Съотношение символи към токени:

- English: 19/6 ≈ 3.2
- Bulgarian: 17/13 ≈ 1.3
- Chinese: 7/10 = 0.7

1. Английски е най-ефективен (най-високо съотношение).
2. При 4096 токена:
...
1. `Ġ` означава space преди токена (GPT-2 моделът кодира интервалите като част от следващия токен).
...
1. English news се компресира най-добре (4.0 символа за токен).
2. Кодът съдържа много специални символи, скоби, и променливи имена, които не се срещат често в тренировъчните данни.
3. BPE работи лошо за:
   - Случайни данни (DNA, криптирани данни)
   - Езици с много малко тренировъчни данни
